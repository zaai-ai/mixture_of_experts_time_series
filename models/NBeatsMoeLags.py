...
class NBEATSMoEBlock(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.nr_experts):
            ...
            nr_lags = input_size if self.list_of_lags is None else self.list_of_lags[i]
            ...
            hidden_layers = [
                nn.Linear(in_features=nr_lags, out_features=mlp_units[0][0], bias=False)
            ]
        ...
        if not self.share_experts:
            self.pooling = SparsePooling(
                ...
                bias=self.bias_load_balancer, list_of_lags=self.list_of_lags,
            )
        else:
...
class NBeatsMoeLags(BaseWindows):
    """NBEATS
    ...
    **References:**<br>
    ...
    def __init__(
        ...
        **trainer_kwargs,
        ...
        super(NBeatsMoeLags, self).__init__(
            h=h,
            ...
            for block_id in range(n_blocks[i]):
                ...
                if shared_weights and block_id > 0:
                    ...
                    nbeats_block = NBEATSMoEBlock(
                        ...
                        list_of_lags=[input_size * (i + 1) // self.nr_experts for i in range(self.nr_experts)],
                    )

                    print([input_size * (i + 1) // self.nr_experts for i in range(self.nr_experts)])
